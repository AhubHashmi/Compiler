...
{
    ifstream code("input-output\\input.txt");
...
{
    string doublePrevChar(1, line[index - 2]);
    string prevChar(1, line[index - 1]);
    string nextChar(1, line[index + 1]);
...
void Lexical::handleEquals(string c, string line, int index)
{
    ...
    if (index > 0)
    {
        ...
        {
            token.pop_back();
            token.push_back(make_tuple("-", prevChar + c, lineNumber));
        }
        else if (prevChar == "=" && EE == true)
        {
            token.pop_back();
            token.push_back(make_tuple("-", prevChar + c, lineNumber));
            EE = false;
